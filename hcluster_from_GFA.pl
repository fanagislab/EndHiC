...
#read from GFA format file
#Only output clusters with >= 2 nodes
#if two nodes has a link, assign distance 0 to them 
...
while (<IN>) {
	...
	next if($point1 eq $point2);
	next if(exists $SAMPLE_DIST{$point2}{$point1});
	$SAMPLE_DIST{$point1}{$point2} = $distance;
	$SAMPLE{$point1} = 1;
	$SAMPLE{$point2} = 1;
...
while (1) {
	...
	foreach my $gid (sort {$a<=>$b} keys %GROUP) {
		...
		next if($dist1 == 1 && $dist2 == 1); ##减少无效计算量
...
foreach my $gid (sort {$sorting{$b} <=> $sorting{$a}} keys %sorting) {
	...
	last if($child_num <= 1);
	$output .= "Group_$cluster_id\t$child_num";
